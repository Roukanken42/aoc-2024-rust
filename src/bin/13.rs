...
fn parse_location(sep: char) -> impl FnMut(&str) -> IResult<&str, Location<i64>> {
    move |input: &str| {
        let parse_x = preceded(pair(tag("X"), char(sep)), i64::parse);
        let parse_y = preceded(pair(tag("Y"), char(sep)), i64::parse);

        map(separated_pair(parse_x, tag(", "), parse_y), |(x, y)| Location::new(x, y))(input)
    }
}
...
pub fn part_two(input: &str) -> Option<i64> {
    let (_, machines) = parse(input).unwrap();
    let raise = Location::new(10000000000000, 10000000000000);

    Some(
        machines
            .iter()
            .flat_map(|machine| solve(machine.button_a, machine.button_b, machine.prize + raise))
            .sum(),
    )
}
